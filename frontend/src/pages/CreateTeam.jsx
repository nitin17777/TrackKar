...
export default function TeamDashboard() {
  ...
  const project = state?.project || { name: "Demo Project", owner: "You", code: code || "DEMO01", members: ["You"], deadline: "" };
  ...
  const addTask = () => {
    ...
    setTasks(prev => [...prev, {
      ...
      assign: taskAssign, priority: taskPriority, status: "todo",
    }]);
    setTaskText(""); setTaskAssign(""); setTaskPriority("medium");
    setAddOpen(false);
  };

  const moveTask   = (id, s) => setTasks(p => p.map(t => t.id === id ? { ...t, status: s } : t));
